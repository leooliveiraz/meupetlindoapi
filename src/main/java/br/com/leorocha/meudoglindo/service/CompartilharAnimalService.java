...
public class CompartilharAnimalService {
    ...
    public String gerarCodigo(Integer idAnimal, String permissao) throws AuthenticationException {
        ...
        CompartilhamentoAnimal compartilhamentoAnimal = new CompartilhamentoAnimal(null, animal, codigo, LocalDateTime.now(), StatusCompartilhamento.AGUARDANDO,
                PermissaoCompartilhamento.valueOf(permissao),
                null);
    ...
    public List<Animal> listarCompartilhadosPorUsuario(PermissaoCompartilhamento permissao) throws AuthenticationException {
        Usuario usuario = usuarioService.buscarPorSub(requestService.getUserDTO().getSub());
        if (usuario == null)
            throw new AuthenticationException("Você precisa estar logado.");
        List<CompartilhamentoAnimal> listCompartilhamento = repository.findByUsuarioIdAndStatusAndPermissao(usuario.getId(), StatusCompartilhamento.COMPARTILHADO, permissao);
    ...
    public Boolean estaCompartilhado(Integer idAnimal, Integer idUsuario, PermissaoCompartilhamento permissao){
        return this.repository.existsByUsuarioIdAndAnimalIdAndStatusAndPermissao(idUsuario,idAnimal, StatusCompartilhamento.COMPARTILHADO, permissao);
    }
}
